...
- **App name:** **Kinlight** (brand decision made June 2026 — supersedes the "Emergency Exit" working name)
  ...
  - **F72b (backend) — next** — ReportLab server-side PDF, Resend sender name + verified domain `kinlight.app`, and all four email functions (`send_notification_email`, `send_allclear_email`, `send_nomination_email`, `send_reminder_email`). Gate: sequence before F63's nomination email reaches new contacts.
...
>
> **Branding update June 2026:** App renamed **Emergency Exit → Kinlight** (name + Lantern logo + lighthouse/harbour/anchor theme; see `kinlight-brand-guide.html` and `kinlight-logo-options-r2.html`). Code-level rename tracked as **F72** — **F72a (frontend) ✅ done; F72b (backend) in progress.** **`kinlight.app` purchased ✅ (Cloudflare, June 2026).** Remaining pre-launch admin: TM check (F75), ASIC name (F76), social handles (F73), .com.au (F74). **F68 is absorbed into F72** (the verified Resend domain becomes `kinlight.app`). Sequence F72 before F63's nomination email reaches new contacts so their first contact comes from Kinlight.
...
| F72 | Rebrand "Emergency Exit" → "Kinlight" | Must | in-progress | **F72a ✅ (frontend, June 2026):** page title, Lantern SVG logo (night-tile icon + `kin`**light** wordmark) in header + login, status/hero/toast copy ("Your light is on" / "Light on" / "Not lit yet"), milestone modal, "How Kinlight works" explainer, client-side jsPDF — name + full palette (charcoal/sage replacing navy/teal). Both `index.html` copies synced. Storage keys (`ee_v3`, `ee_onboarded`, `ee_first_checkin_done`) intentionally preserved. **F72b (backend — next):** ReportLab server-side PDF; Resend sender name → "Kinlight"; verified domain `kinlight.app` (absorbs F68); email subjects/bodies in `send_notification_email`, `send_allclear_email`, `send_nomination_email`, `send_reminder_email`. **Gated on:** Resend domain verification + TM/ASIC (Anggi action). Sequence F72b before F63's nomination email. |
